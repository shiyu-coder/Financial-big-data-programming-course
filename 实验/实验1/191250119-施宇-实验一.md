...
对于程序一，每个线程执行的运算的时间成本大于线程间的切换和通讯成本，约等于进程间通讯成本。因此当进程数固定时，随着线程数的增加，每个线程的运行时间相应缩短，程序的运行速度加快；当线程数固定时，由于进行数增加带来的额外开销与每个线程的计算成本几乎相同，因此随着进程数的增加，程序的运行速度没有明显的变化。对于程序二，多线程数能够加快程序的运行速度，而进程数的增加对程序运行速度的提升没有明显效果。
